...
if(!defined('IN_UC')){
	include_once S_ROOT.'./config.inc.php';
  error_reporting(0);
  ...
  $code = @$_GET['code'];
	...
	} elseif($timestamp - $_GET['time'] > 3600) {
...
class uc_note {
    ...
    Yii::createWebApplication($config);
		$user=new User();
		$user_datas=$user->find(array('select'=>'id','condition'=>'user_login=:user_login','params'=>array(':user_login'=>$usernameold)));
		if(!empty($user_datas)){
	     	  $user_id=$user_datas->id;
	     	  $update_datas['user_login']=$usernamenew;
	     	  $user->update_table_datas($user_id,$update_datas);
	  }
		return API_RETURN_SUCCEED;
	}
    ...
    Yii::createWebApplication($config);
    $model=new User();
    $user_datas=$model->find(array('select'=>'id,user_login','condition'=>'user_login=:user_login','params'=>array(':user_login'=>$username)));
    
    if(!empty($user_datas)){
    
      Yii::app()->user->setId($user_datas->id);
      Yii::app()->user->setName($user_datas->user_login);
      Yii::app()->user->saveToCookie(3600*1);
    }
    ...
    Yii::createWebApplication($config);
		...
		$user_datas=$user_registe->find(array('select'=>'id,salt','condition'=>'user_login=:user_login','params'=>array(':user_login'=>$username)));
		if(!empty($user_datas)){
	     	  $user_id=$user_datas->id;
	     	  $salt=Util::randStr(6);
	     	  $update_datas['password']=Util::hc($password,$salt);
	...
	function getcreditsettings($get, $post) {
		...
		require_once $this->appdir.'./uc_client/data/cache/creditssettings.php';
		$credits = array();
		foreach($_CACHE['settings']['extcredits'] as $id => $extcredits) {
	...
	$cachefile = $this->appdir.'./uc_client/data/cache/creditssettings.php';
		$fp = fopen($cachefile, 'w');
		$s = "<?php\r\n";
		$s .= '$_CACHE[\'settings\'] = '.arrayeval($outextcredits).";\r\n";
		fwrite($fp, $s);
		fclose($fp);
