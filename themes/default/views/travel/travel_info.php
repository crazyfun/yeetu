    ...
    <div class="order_main1">
      <div class="clear_float"></div>
       <?php  
          ...
          if($this->trave_category!='2'){
          	echo CHtml::beginForm($this->createUrl("travel/insertti"),"POST",array("id"=>'insertti','onsubmit'=>'return validate_contact();'));
  
          }else{
          	if(empty($insurance_ids)){
          	 echo CHtml::beginForm($this->createUrl("travel/insertti"),"POST",array("id"=>'insertti','onsubmit'=>'return validate_peripheral_contact();'));
          	}else{
          	 echo CHtml::beginForm($this->createUrl("travel/insertti"),"POST",array("id"=>'insertti','onsubmit'=>'return validate_contact();'));
          ...
          echo CHtml::hiddenField("hidden_adult_price",$adult_price,array('id'=>'hidden_adult_price'));
          echo CHtml::hiddenField("hidden_child_price",$child_price,array('id'=>'hidden_child_price'));
          echo CHtml::hiddenField("hidden_start_date",$start_time,array('id'=>'hidden_start_date'));
          echo CHtml::hiddenField("hidden_total_price",$total_price,array('id'=>'hidden_total_price'));
      ...
      <div class="order_title">游客信息<span class="cyellow ">请准确填写游客信息，以免在办理出游手续时发生问题。儿童游客如无相关证件，证件类型请选择“其他”，并填写出生日期。</span></div>
        ...
        <tr>
          ...
          <td>
           ...
           证件说明 <a class="trave_tipsy" title="购票或办理登机需要的有效证件类型：&#13中国大陆籍乘客的居民身份证、护照、港澳通行证、军官证、武警警官证、士兵证、军队学员、军队文职干部证、军队离退休干部证和军队职工证。&#1316岁以下未成年人可凭其户口簿或户口所在地公安机关出具的身份证么登机；&#13港、澳地区居民和台湾同胞的回乡证、台胞证；外籍乘客的护照、旅行证、外交官证等；民航总局规定的其他有效登机身份证件。&#13安检时，工作人员将何时机票上姓名与证件的一致性，且只能使用对应的证件原件乘机，&#13否则请影响您顺利登机，请认真填写证件内容并核实，出游时请携带好有效证件原件。"><img src="/css/images/qus.gif"/></a> </td>
        </tr>
...
?>
  ...
  <script language="javascript">
    ...
    if(!adult_nums){
		...
		//验证周边游信息表单
	function validate_peripheral_contact(){
