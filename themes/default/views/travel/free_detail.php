<?php
    ...
    Yii::app()->clientScript->registerScriptFile('/js/lightbox/jquery.lightbox.js');
		if($this->beginCache("travefreedetails", array('dependency'=>array(
...
<div class="wj_mainL"><!--内容左边部分-->
    	<div class="wj_toptab">
            <div class="wj_txtu">
                <span style="float:left;"><span><?php $trave_district=new District(); $trave_sregion_data=$trave_district->get_table_datas($model->trave_sregion);$trave_region_data=$trave_district->get_table_datas($model->trave_region);echo $trave_sregion_data['district_name']."/".$trave_region_data['district_name']; ?></span><span class="wj_redtxt">编号<?php echo $model->trave_number;?></span><span>本产品由<?php if($model->trave_category=='1')  echo $model->Agency->agency_name; else echo "上海易途旅行社"; ?>提供相关服务</span></span><span style="float:right;"><div id="ckepop">
        ...
        <table class="DIYtour_m_table">
          ...
          <tbody>
            ...
            <tr>
              <td>游客信息</td>
              <td>
               <span class="wj_startdate"><?php echo CHtml::textField("adult_nums","",array('class'=>'wj_crinput',"onkeyup"=>"javascript:isNumber(this);"));?></span><span class="wj_startdate">成人</span><span class="wj_startdate"><?php echo CHtml::textField("child_nums","",array('class'=>'wj_crinput',"onkeyup"=>"javascript:isNumber(this);"));?>儿童</span>
                </td>
 ...
 <script language="javascript">
 	...
 	jQuery(document).ready(function(){
 		...
 		if(tc){
 		 ...
 		 get_trave_flight(trave_id,trave_date.date_key,trave_date.std_value);
 		 get_trave_hotels(trave_id,trave_date.std_value,1);

 		 jQuery("#select_trave_date").change(function(){
 		    	 var trave_date=get_trave_date_datas();
 		    	 std_value=trave_date.std_value;
 		    	 get_trave_flight(trave_id,trave_date.date_key,trave_date.std_value);
 		       get_trave_hotels(trave_id,std_value,1);
 		 });
  ...
  function calculation_free_total(){
    var trave_date=get_trave_date_datas();
  	...
  	var total_price=(parseInt(trave_date.adult_price)*t_adult_nums)+(parseInt(trave_date.child_price)*t_child_nums);
  	if(!t_total_nums){
  		var people_price=0;
  	}else{
