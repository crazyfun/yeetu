...
class Image_GD_Driver extends Image_Driver {
	...
	public function crop_resize($properties){
		$width = imagesx($this->tmp_image);
		$height = imagesy($this->tmp_image);
		$properties_width=$properties['width'];
	  $properties_height=$properties['height'];
	  $properties_scale=$properties_width/$properties_height;
	  $tem_scale=$width/$height;

	  if($tem_scale>$properties_scale){
	  	  $properties_width=round($properties_scale*$height);
	  	  $properties_height=$height;
	  	  
	  }
	  if($tem_scale<$properties_scale){
	  	$properties_height=round($width/$properties_scale);
	  	$properties_width=$width;
	  }
	  $properties['width']=$properties_width;
	  $properties['height']=$properties_height;
	  return $properties;
	...
	{
		$crop_properties=$this->crop_resize($properties);
		$this->crop($crop_properties);
		// Get the current width and height
		$width = imagesx($this->tmp_image);
		$height = imagesy($this->tmp_image);
