<?php
class TravelfinfoAction extends  BaseAction{
  ...
  protected function do_action(){
	   ...
	   $room_nums=$_REQUEST['hidden_room_nums'];
		 ...
		 $totel_nums=intval($adult_nums)+intval($child_nums);
		  if(empty($trave_id)||empty($hidden_total_price)||empty($select_trave_date)||empty($totel_nums)){
      ...
      $this->controller->pt($this->id,array($trave_datas->trave_name.'旅游线路在线预定_游客信息-易途旅游网'));
        ...
        foreach($visitor_datas as $key => $value){
        	$travel_people[$ii]['real_name']=$value['contact_name'];
		  	...
		  	$this->display('travel_finfo',array('model'=>$ordercontact,'trave_id'=>$trave_id,'select_trave_date'=>$select_trave_date,'adult_nums'=>$adult_nums,'child_nums'=>$child_nums,'room_nums'=>$room_nums,'hidden_room_id'=>$hidden_room_id,'hidden_hotel_id'=>$hidden_hotel_id,'hidden_trave_route_number'=>$hidden_trave_route_number,'hidden_total_price'=>$hidden_total_price,'insurance_ids'=>$insurance_ids,'travel_people'=>$travel_people,'start_date_id'=>$start_date_id));
