<?php
class InsertftiAction extends  BaseAction{
  ...
  protected function do_action(){
		 ...
		 $select_trave_date=$_REQUEST['hidden_start_date'];
		 $adult_nums=$_REQUEST['hidden_adult_nums'];
		 $child_nums=$_REQUEST['hidden_child_nums'];
	   $room_nums=$_REQUEST['hidden_room_nums'];
		 ...
		 $totel_nums=intval($adult_nums)+intval($child_nums);
		 if(empty($trave_id)||empty($totel_nums)||empty($select_trave_date)||empty($hidden_total_price)){
			  $this->controller->redirect($this->controller->createUrl('error/errororder',array()));
		 }
		  $travel_people_json=$_POST['travel_people_json'];
			switch($travel_action){
        	 ...
        	 $errors=$ordercontact->getErrors();
		        ...
		        $total_coupon_value=$this->get_coupon_value($trave_id,$adult_nums);
				    $this->display('travel_fcheck',array('trave_datas'=>$trave_datas,'trave_id'=>$trave_id,'select_trave_date'=>$select_trave_date,'adult_nums'=>$adult_nums,'child_nums'=>$child_nums,'room_nums'=>$room_nums,'hidden_room_id'=>$hidden_room_id,'hidden_hotel_id'=>$hidden_hotel_id,'hidden_trave_route_number'=>$hidden_trave_route_number,'hidden_total_price'=>$hidden_total_price,'insurance_ids'=>$insurance_ids,'token_key'=>$token_key,'travel_people'=>$travel_people,'travel_people_json'=>$travel_people_json,'user_datas'=>$user_datas,'total_coupon_value'=>$total_coupon_value,'start_date_id'=>$start_date_id));
				   }else{
				      $this->display('travel_finfo',array('model'=>$ordercontact,'trave_id'=>$trave_id,'select_trave_date'=>$select_trave_date,'adult_nums'=>$adult_nums,'child_nums'=>$child_nums,'room_nums'=>$room_nums,'hidden_room_id'=>$hidden_room_id,'hidden_hotel_id'=>$hidden_hotel_id,'hidden_trave_route_number'=>$hidden_trave_route_number,'hidden_total_price'=>$hidden_total_price,'insurance_ids'=>$insurance_ids,'travel_people'=>$travel_people,'errors'=>$errors,'start_date_id'=>$start_date_id));
			     }
					   break;
					case '2':
					$trave=new Trave();
      	  ...
      	  $user_id=Yii::app()->user->id;
					$user=new User();
		      $user_datas=$user->get_table_datas($user_id);
      	 	$this->controller->check_free_order_error($hidden_total_price,$trave_id,$start_date_id,$select_trave_date,$adult_nums,$child_nums,$insurance_ids,$room_nums,$hidden_room_id,$hidden_trave_route_number);
		            ...
		            if($coupon_value > $total_coupon_value){
		            	  ...
		            	  $this->display('travel_fcheck',array('trave_datas'=>$trave_datas,'trave_id'=>$trave_id,'select_trave_date'=>$select_trave_date,'hidden_total_price'=>$hidden_total_price,'insurance_ids'=>$insurance_ids,'adult_nums'=>$adult_nums,'child_nums'=>$child_nums,'adult_price'=>$adult_price,'child_price'=>$child_price,'token_key'=>$token_key,'travel_people'=>$travel_people,'travel_people_json'=>$travel_people_json,'user_datas'=>$user_datas,'total_coupon_value'=>$total_coupon_value,'start_date_id'=>$start_date_id,'room_nums'=>$room_nums,'hidden_room_id'=>$hidden_room_id,'hidden_hotel_id'=>$hidden_hotel_id,'hidden_trave_route_number'=>$hidden_trave_route_number,'order_remark'=>$order_remark));
		            	  exit();
		            }else{
		            	if($coupon_value > $user_coupon){
		              	...
		              	if($coupon_value > $hidden_total_price){
		  	      ...
		  	      $total_price=$hidden_total_price-intval($coupon_value);
		  				...
		  				$trave_order->start_date=$select_trave_date;
		  				$trave_order->total_price=$total_price;
		  				$trave_order->coupon_value=$coupon_value;
		  				$trave_order->room_nums=$room_nums;
		  				$trave_order->trave_route_number=$hidden_trave_route_number;
		  				$trave_order->room_id=$hidden_room_id;
              ...
              $trave_order->suppliers_settle='1';
                ...
                $send_mail=new SendMail("Make Order");
                 $validate_flag=$user_datas->validate_user_email();
		             if($validate_flag){
		                  $send_mail->send_order_success_mail("",$user_datas->email,$trave_datas->trave_name,$select_trave_date,$total_price,$user_datas->user_login,$email_total_nums);
		             }
                        ...
                        $order_contact->valid_date=$value['valid_year']."-".$value['valid_month']."-".$value['valid_day'];
		  	 				 ...
		  	 				 $this->display('travel_pay',array('trave_id'=>$trave_id,'order_id'=>$order_id,'total_price'=>$total_price));
		 					 }else{
